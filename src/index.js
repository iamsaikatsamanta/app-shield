...
program.version(`codelock ${require("../package").version}`).usage("<command>");
  ...
  .action((name, options) => {
    ...
    if (name && JSON.stringify(name) !== "{}") {
      ...
      if (
        ...
        credentials.project_id === "" ||
        !credentials.scan_frequency ||
        Number(credentials.scan_frequency) > 59 ||
        Number(credentials.scan_frequency) < 1
      ) {
...
program.command("scan").action(async (name) => {
  try {
    const existingConfig = fs.readFileSync(configPath, "utf8");
    const credentials = JSON.parse(existingConfig);
    const hash = await Config.triggerScan(
      credentials.hash_function,
      credentials.build_path
    );
...
program.command("unlink").action(async (name) => {
  try {
    const existingConfig = fs.existsSync(configPath);
    if (existingConfig) {
      const existingConfig = fs.readFileSync(configPath, "utf8");
