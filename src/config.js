  ...
  inquirer
    .prompt([
      ...
      },
      {
        ...
        type: "number",
        message:
          "Please enter how freequent you wnat to scan your code in minutes? (1-59)",
...
const initProject = (response) => {
  ...
  if (
    Number(response.scan_frequency) > 59 ||
    Number(response.scan_frequency) < 1
  ) {
    ...
    .then((response) => {
      if (response.code === 0) {
        fs.writeFileSync(
          ...
          JSON.stringify({
            ...credentials,
            hash_function: response.result.hash_function,
          })
        );
        return triggerScan(
          response.result.hash_function,
          credentials.build_path
        );
      } else {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const data = await axios.post(
        "https://test.api.codelock.ai/api/v1/configure-project",
        {
          project_id: codelock_config.project_id,
          build_path: codelock_config.build_path,
          scan_frequency: codelock_config.scan_frequency,
        },
        {
          auth: {
  ...
  new Promise(async (resolve, reject) => {
    try {
      ...
      if (codelock_config) {
        ...
        const data = await axios.post(
          "https://test.api.codelock.ai/api/v1/add-project-scan",
          {
        ...
        reject(
          "Unable to locate configration file try with codelcok init again"
        );
      }
    } catch (error) {
      reject(error);
